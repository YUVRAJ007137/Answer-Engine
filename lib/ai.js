...
DIRECT MATH / CALCULATION RULE (follow this when the question is only asking for a result or a table):
- If the question is a direct math request (e.g. "what is 2+2", "solve 5*3", "table of 2", "table of 5", "calculate ...", "compute ...", simple arithmetic, multiplication table, percentage, one-step calculation), respond with ONLY the answer. Do NOT use Definition, Explanation, Example, or Conclusion. Do NOT explain or add any extra text.
- Examples: "what is 2+2?" → answer: $2+2=4$. "Table of 2" → answer: only the table (2×1=2, 2×2=4, 2×3=6, ...). "15% of 200" → answer: $15\\% \\times 200 = 30$. "Solve 3x=9" → answer: $x=3$.
- For multiplication tables, list only the table entries (e.g. 2×1=2, 2×2=4, ... up to 2×10=20). No intro or conclusion.

For all other (non-direct-math) questions, follow the guidelines below.
...
Math (when the question involves mathematics, formulas, or equations):
- Use LaTeX so equations render correctly. Inline: single dollar signs, e.g. $E = mc^2$, $x^2 + y^2 = z^2$.
- Display (block) equations: double dollar signs on their own lines, e.g. $$\\frac{a}{b}$$ or $$\\int_0^1 x^2 \\, dx$$.
- Use standard LaTeX: \\frac, \\sqrt, \\sum, \\int, \\alpha, \\beta, subscripts x_i, superscripts x^2, matrices, etc.
...
function formatContextBlock(previousQA) {
  if (!previousQA?.length) return "";
  const lines = [
    "Context from this session (use this to resolve pronouns like \"it\", \"this\", \"that\" in the current question):",
    "",
  ];
  const start = Math.max(0, previousQA.length - MAX_CONTEXT_PAIRS);
  for (let j = start; j < previousQA.length; j++) {
    const qa = previousQA[j];
    const ans = qa.answer && qa.answer.length > MAX_ANSWER_CHARS
      ? qa.answer.slice(0, MAX_ANSWER_CHARS) + "…"
      : (qa.answer || "");
    lines.push(`Question ${j + 1}: ${qa.question || ""}`);
    lines.push(`Answer ${j + 1}: ${ans}`);
    lines.push("");
  }
  lines.push("Current question (resolve any pronouns using the context above):");
  lines.push("");
  return lines.join("\n");
}
...
function buildPrompt(question, lengthKey, styleKey, context) {
  const length = LENGTH_SPEC[lengthKey] || LENGTH_SPEC.standard;
  const style = STYLE_SPEC[styleKey] || STYLE_SPEC.simple;
  const contextBlock = formatContextBlock(context?.previousQA);
...
export async function generateAnswer(question, options = {}, context = {}) {
  ...
  const prompt = buildPrompt(question, length, style, context);
  ...
  try {
    ...
    if (isRateLimitError(err) && retryCount < MAX_RETRIES) {
      ...
      await sleep(delayMs);
      return generateAnswer(question, { ...options, retryCount: retryCount + 1 }, context);
    }
    throw err;
  }
}
